...
# pytest, and the /wrap-up discipline-test gate (cd svp_pipeline &&
# python -m pytest tests/discipline/ -q) work in Claude Code on the web
...
# container state.
log="$(mktemp)"
if ! python -m pip install -q -e "$CLAUDE_PROJECT_DIR/svp_pipeline[dev]" >"$log" 2>&1; then
  echo "session-start: pip install -e \"$CLAUDE_PROJECT_DIR/svp_pipeline[dev]\" failed:" >&2
  cat "$log" >&2
  exit 1
fi
