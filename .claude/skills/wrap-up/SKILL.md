---
name: wrap-up
description: Persist a session-end reflection into .claude/memory and run the memory-hygiene sweep for the svp-video-pipeline repo. Use when the user signals the session is ending — e.g. 「今日はここまで」「今日は終わり」「セッション終了」「また明日」「お疲れ様」「done for today」「that's all」 — or runs /wrap-up manually.
---
...
- **step 8 (`cd svp_pipeline && python -m pytest tests/discipline/ -q`) must
  run before any direct push** — the `.claude/memory/` main-push exception
  is post-hoc-only, so a discipline violation turns main red directly
  instead of being blocked by PR CI.
...
**Fable/メイン直接実行監査**（CLAUDE.md § Advisor Strategy 運用細則の事後
監査）: メインエージェント稼働セッションでは、Artifacts か Handoff に
「メインが直接実行した手を動かす作業」（マイクロ操作例外＝1–2 ツール
コールを超えた実装・実行・検証・調査）を 1–2 行で明記する。委譲すべき
だった項目はそのまま次セッションへの改善引き継ぎになる。該当なしなら
その旨 1 語で足りる。
...
by `svp_pipeline/tests/discipline/test_index_md_entry_compactness.py`. Do
NOT essay-ify the entry; full narrative lives in the dated file.
...
`svp_pipeline/tests/discipline/test_status_md_next_queue_no_completed.py`.
...
`svp_pipeline/tests/discipline/test_status_md_phase_single_paragraph.py`.
...
drift remains from steps 4–6 (or `CLAUDE.md` grew past its 400-line cap, or
a `README.md` grew past its 350-line hard limit — see `test_claude_md_line_
cap.py` / `test_readme_line_cap.py`) — fix the offending file and re-run;
do NOT push red. Only `.claude/memory/` changes may go direct to main (the
...
- **README.md（ルート または `svp_pipeline/README.md`）を 350 行超に肥大させる**
  (README は入口情報限定の役割を逸脱。 30 行超の section は `docs/<topic>.md`
  へ抽出しリンク化する。 `test_readme_line_cap.py` で enforce)。
